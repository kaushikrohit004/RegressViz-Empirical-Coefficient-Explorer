...
The **goal** of `RegressViz` is to **enhance the interpretation** of
regression results by providing **visualizations** that integrate
empirical data distributions. This approach facilitates a more thorough
...
[GitHub](https://github.com/kaushikrohit004/RegressViz-Empirical-Coefficient-Explorer)
with:
...
$$P\left( Y = 1|\mathbf{X} \right) = \pi(\mathbf{X}) = \frac{\exp\left( \beta_{0} + \beta_{1}X_{1} + \ldots + \beta_{n}X_{n} \right)}{1 + \exp\left( \beta_{0} + \beta_{1}X_{1} + \ldots + \beta_{n}X_{n} \right)} = \frac{\exp(\mathbf{X}\beta)}{1 + \exp(\mathbf{X}\beta)} = \frac{1}{1 + \exp( - \mathbf{X}\beta)}$$

Odds Ratio is:

$$OR = \frac{\pi(\mathbf{X})}{1 - \pi(\mathbf{X})} = \frac{\frac{\exp(\mathbf{X}\beta)}{1 + \exp(\mathbf{X}\beta)}}{1 - \frac{\exp(\mathbf{X}\beta)}{1 + \exp(\mathbf{X}\beta)}} = \exp\left( \beta_{0} + \beta_{1}X_{1} + \ldots + \beta_{n}X_{n} \right)$$
...
$$\log\frac{P\left( Y = 1|\mathbf{X}_{\mathbf{X}_{\mathbf{1}} = \mathbf{X}_{\mathbf{1}} + \mathbf{1}} \right)}{P\left( Y = 0|\mathbf{X}_{\mathbf{X}_{\mathbf{1}} = \mathbf{X}_{\mathbf{1}} + \mathbf{1}} \right)} - \log\frac{P\left( Y = 1|\mathbf{X}_{\mathbf{X}_{\mathbf{1}} = \mathbf{X}_{\mathbf{1}}} \right)}{P\left( Y = 0|\mathbf{X}_{\mathbf{X}_{\mathbf{1}} = \mathbf{X}_{\mathbf{1}}} \right)}\overset{1}{=}$$

$$\overset{1}{=}\beta_{0} + \beta_{1}\left( X_{1} + 1 \right) + \ldots + \beta_{n}X_{n} - \left( \beta_{0} + \beta_{1}X_{1} + \ldots + \beta_{n}X_{n} \right) = \beta_{1}$$

Therefore, coefficient $\beta_{1}$ shows expected change in the Log Odds
for a one unit increase in $X_{1}$. Thus, expected change in the Odds
Ratio is $\exp\left( \beta_{1} \right)$. Finally, expected change in the
Odds Ratio if $X_{1}$ changes by k units whilst all other variables
remain unchanged is
$\left\lbrack \exp\left( \beta_{1} \right) \right\rbrack^{k}$ or
$\exp\left( \beta_{1} \times k \right)$.
...
![](reference/figures/README-plot-Odds-Ratio-1.png)
...
![](reference/figures/README-unnamed-chunk-3-1.png) As observed, when
returning individual plots, the resulting entities are `ggplot` objects.
Consequently, any operation that is compatible with ggplot can be
...
$$E\left\lbrack Y|\textbf{𝐗} \right\rbrack = \beta_{0} + \beta_{1}x_{1} + \beta_{2}x_{2} + \beta_{12}x_{1}x_{2}$$

Define the **marginal effect** by taking the partial derivative with
respect to $x_{2}$:

$$\gamma_{2} = \frac{\partial E\left\lbrack Y|\textbf{𝐗} \right\rbrack}{\partial x_{2}} = \beta_{2}$$

Therefore, $\beta_{2}$ is sufficient to quantify how much
$E\left\lbrack Y|\textbf{𝐗} \right\rbrack$ changes with respect to every
one unit increase in $\beta_{2}$, holding all other variables constant.

Now, take the second order cross-partial derivative of
$E\left\lbrack Y|\textbf{𝐗} \right\rbrack$ with respect to both $x_{1}$
and $x_{2}$:

$$\gamma_{12}^{2} = \frac{\partial^{2}E\left\lbrack Y|\textbf{𝐗} \right\rbrack}{\partial x_{1}\partial x_{2}} = \beta_{12}$$

Similar intuition as above holds. The interaction term $\beta_{12}$
shows how effect of $x_{1}$ on
$E\left\lbrack Y|\textbf{𝐗} \right\rbrack$ changes for every one unit
increase in $x_{2}$ and vice versa.
...
$$g\left( E\left\lbrack Y|\textbf{𝐗} \right\rbrack \right) = \beta_{0} + \beta_{1}x_{1} + \beta_{2}x_{2} + \beta_{12}x_{1}x_{2}$$

Converting GLM to a natural scale using the inverse link function:

$$E\left\lbrack Y|\textbf{𝐗} \right\rbrack = g^{- 1}\left( \beta_{0} + \beta_{1}x_{1} + \beta_{2}x_{2} + \beta_{12}x_{1}x_{2} \right)$$

Note that the relationship is **no longer linear**.

As an example, consider logistic regression:

$$\log\left( \frac{E\left\lbrack Y|\textbf{𝐗} \right\rbrack}{1 - E\left\lbrack Y|\textbf{𝐗} \right\rbrack} \right) = \beta_{0} + \beta_{1}x_{1} + \beta_{2}x_{2} + \beta_{12}x_{1}x_{2} = \eta$$

Transformation leads to:

$$E\left\lbrack Y|\textbf{𝐗} \right\rbrack = \frac{1}{1 + exp\left( - \{\beta_{0} + \beta_{1}x_{1} + \beta_{2}x_{2} + \beta_{12}x_{1}x_{2}\} \right)} = \frac{1}{1 + exp( - \eta)} = \frac{exp(\eta)}{1 + exp(\eta)}$$

Let’s take the second order cross-partial derivative.

Using the chain rule:

$$\gamma_{12}^{2} = \frac{\partial^{2}E\left\lbrack Y|\textbf{𝐗} \right\rbrack}{\partial x_{1}\partial x_{2}} = \frac{\partial^{2}g^{- 1}(\eta)}{\partial x_{1}\partial x_{2}} = \frac{\partial}{\partial x_{1}}\left\lbrack \frac{\partial g^{- 1}(\eta)}{\partial x_{2}} \right\rbrack\overset{2}{=}$$

$$\overset{2}{=}\frac{\partial}{\partial x_{1}}\left\lbrack \frac{\partial g^{- 1}(\eta)}{\partial\eta}\frac{\partial\eta}{\partial x_{2}} \right\rbrack = \frac{\partial}{\partial x_{1}}\left\lbrack \left( \beta_{2} + \beta_{12}x_{1} \right){\dot{g}}^{- 1}(\eta) \right\rbrack$$

Utilizing the product rule followed by the chain rule:

$$\frac{\partial}{\partial x_{1}}\left\lbrack \left( \beta_{2} + \beta_{12}x_{1} \right){\dot{g}}^{- 1}(\eta) \right\rbrack = \frac{\partial}{\partial x_{1}}\left\lbrack \left( \beta_{2} + \beta_{12}x_{1} \right) \right\rbrack{\dot{g}}^{- 1}(\eta) + \left\lbrack \left( \beta_{2} + \beta_{12}x_{1} \right) \right\rbrack\frac{\partial}{\partial x_{1}}\left\lbrack {\dot{g}}^{- 1}(\eta) \right\rbrack\overset{3}{=}$$

$$\overset{3}{=}\beta_{12}{\dot{g}}^{- 1}(\eta) + \left( \beta_{2} + \beta_{12}x_{1} \right)\left( \beta_{1} + \beta_{12}x_{2} \right){\ddot{g}}^{- 1}(\eta)$$

First and second derivative of the inverse link function are:

$${\dot{g}}^{- 1}(\eta) = \frac{exp(\eta)}{\left( 1 + exp(\eta) \right)^{2}}$$

$${\ddot{g}}^{- 1}(\eta) = \frac{exp(\eta)\left( 1 - exp(\eta) \right)}{\left( 1 + exp(\eta) \right)^{3}}$$

Therefore:

$$\gamma_{12}^{2} = \beta_{12}\frac{e^{\eta}}{\left( 1 + e^{\eta} \right)^{2}} + \left( \beta_{1} + \beta_{12}x_{2} \right)\left( \beta_{2} + \beta_{12}x_{1} \right)\frac{e^{\eta}\left( 1 - e^{\eta} \right)}{\left( 1 + e^{\eta} \right)^{3}}$$
...
variable $x_{1}$ on $E\left\lbrack Y|\textbf{𝐗} \right\rbrack$ changes
for each one-unit increase in the variable $x_{2}$, and vice versa.
...
coefficients and calculate the values of ${\widehat{\gamma}}_{12}^{2}$.
Subsequently, we visualize the slopes of
$\widehat{E}\left\lbrack Y|\mathbf{X} \right\rbrack$ calculated for
several combinations of `X1b` and `X2b`.
...
X_{1} \\
X_{2}
\end{pmatrix} \sim \mathcal{N}\left( \begin{matrix}
\begin{pmatrix}
\mu_{1} \\
\mu_{2}
\end{pmatrix}
...
$$\phi\left( x_{1},x_{2} \right) = \frac{1}{2\pi\sigma_{1}\sigma_{2}\sqrt{1 - \rho^{2}}}\exp\left( - \frac{1}{2\left( 1 - \rho^{2} \right)}\left\lbrack \left( \frac{x_{1} - \mu_{1}}{\sigma_{1}} \right)^{2} - 2\rho\left( \frac{x_{1} - \mu_{1}}{\sigma_{1}} \right)\left( \frac{x_{2} - \mu_{2}}{\sigma_{2}} \right) + \left( \frac{x_{2} - \mu_{2}}{\sigma_{2}} \right)^{2} \right\rbrack \right)$$
...
Moreover, the magnitude and sign of ${\widehat{\gamma}}_{12}^{2}$ are
contingent upon the specific combination of `X1b` and `X2b` variables
utilized in the analysis.

#### Visualize changes in $\widehat{E}\left\lbrack Y|\textbf{𝐱} \right\rbrack$ associated with one unit increase in `X1b` at different quantiles of `X2b`.
...
The alterations in $\widehat{E}\left\lbrack Y|\textbf{𝐱} \right\rbrack$
associated with one-unit increments in `X1b` at the first and third
quartiles of `X2b` demonstrate significant disparities. Observe the
variations in $\widehat{E}\left\lbrack Y|\textbf{𝐱} \right\rbrack$ for a
unit change in `X1b` from $- 1$ to $0$ (red) compared to the changes in
$\widehat{E}\left\lbrack Y|\textbf{𝐱} \right\rbrack$ for a unit change
in X1b from $0.5$ to $1.5$ (green). Observe how the magnitudes of
...
assertion regarding the inadequacy of ${\widehat{\beta}}_{12}$ in
capturing the interaction effects on the probability scale within the
GLM framework, which incorporates two continuous predictors and an
